...
| ---------------------- | ------------------- | ------- |
| `NEXT_PUBLIC_API_URL`  | **Yes** (production) | Origin the browser uses for the API, without `/api` (e.g. `https://api.example.com`). Also used for the `connect-src` entry of the Content-Security-Policy. |
| `API_PROXY_TARGET`     | No                  | Server-side only. When set, the app rewrites `/api/*` to `${API_PROXY_TARGET}/api/*` (see [Same-site cookies](#same-site-cookies-and-the-api-proxy)), and server-side metadata/sitemap requests go straight to it. |
| `NEXT_PUBLIC_SITE_URL` | **Yes** (production) | Public origin of this frontend (bare origin, no path). Used for canonical URLs, Open Graph, `/sitemap.xml` and `/robots.txt`. |

`npm run build` **fails** if either public URL is missing, is not an absolute URL, or
uses `http://` on anything other than localhost. That is deliberate: these values are
baked into the bundle, so a missing one would otherwise ship a site whose API calls,
canonical URLs and sitemap all point at `localhost`. The localhost defaults apply to
`npm run dev` only.
...
- `/robots.txt` disallows the private surfaces (`/dashboard`, `/login`,
  `/forgot-password`, `/register`, `/offline`), which also send `noindex` metadata.
  `/search`, `/test` and `/banner/studio` stay crawlable but are `noindex, follow`:
  they are per-visitor or per-query pages, and crawlers should still follow their links.

## SEO

- Titles, descriptions, canonical URLs, Open Graph and Twitter metadata are set in
  `src/app/layout.js` (defaults) and per page. `metadataBase` comes from
  `NEXT_PUBLIC_SITE_URL`, so canonicals and social URLs are absolute.
- The social preview image is generated at build time from
  `src/app/opengraph-image.js` (no static asset to keep in sync).
- Class, book and chapter pages are server components: `generateMetadata()` fetches the
  real name from the API (cached for an hour, 4 s timeout) so each page has its own title
  and description, and they emit `BreadcrumbList` structured data. If the API cannot be
  reached the page still renders, with a generic title and `noindex` so a placeholder is
  never indexed.
- `/sitemap.xml` lists the static pages plus every public class, book and chapter,
  revalidated daily. Archived classes and deleted books are excluded by the API, and if
  the API is unreachable during a build only the static pages are listed.
- The home page carries `WebSite` structured data. No keyword stuffing or invented
  ratings/prices anywhere.

## Search

`/search` searches classes, books, chapters and question text (English **and** Urdu) via
the backend's `GET /api/search`, which is backed by trigram indexes. The input is
debounced (300 ms), requires two characters, and keeps the term in the URL (`?q=`) so a
search can be shared. Archived classes and deleted books never appear in results.
...
- **API responses are never cached**: `/api/*` (same origin, proxy mode) is `NetworkOnly`,
  and so is the API origin when it differs from this site. The question bank and any
  authenticated response therefore never reach Cache Storage.
- **Small first install**: the PDF engine (`/libhtml2realpdf.wasm`, ~8.5 MB), the Urdu
  ...
  fonts are cached (`CacheFirst`) the first time they are used, so a teacher who has
  generated one paper can generate more offline. A teacher who has never generated a
  paper still needs to be online for the first one — warming that cache up front would
  mean ~11 MB of downloads for every visitor, which is not worth it.
- The wasm URL carries `?v=<engine version>` and only one entry is kept, so upgrading
  `@imggion/html2realpdf` replaces the cached engine instead of serving a stale copy
  against new glue code.
